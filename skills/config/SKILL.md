...
|---|---|---|---|---|
| `plugin_version` | string | yes (after 0.3.0) | (set by this skill) | 0.3.0 |
...
Compare the `plugin_version` value in the config (call it `config_version`) against the current plugin version `0.3.0`.

**If `plugin_version` is absent** (v0.1 config -- this field did not exist before 0.3.0):
- This is a non-breaking migration. Auto-patch: add `plugin_version: 0.3.0` at the top of the file (after any leading comment block). No other fields changed in 0.3.0. Report what was added.

**If `config_version == plugin_version`** (up to date):
- Validate all required fields are present and non-null (see schema). Report any missing required fields and offer to add sensible defaults. Otherwise report "config OK at 0.3.0" and stop.

**If `config_version` is a lower minor or patch than `0.3.0`** (e.g. `0.1.x`):
...
**If `config_version` is a higher version than `0.3.0`**:
...
|---|---|---|
| (absent / 0.1.x) | 0.3.0 | Add `plugin_version: 0.3.0` |
...
3. Create `.ai-lore/` if it does not exist. Write `config.yaml` from `templates/config.yaml` with the detected values filled in and `plugin_version: 0.3.0` set. Report the created file path.

### 5. Report

End with a one-line status: `config OK at 0.3.0`, `config migrated 0.1.x -> 0.3.0 (added: plugin_version)`, or `config created at 0.3.0`. If any required fields are still missing after migration/creation (e.g. the user declined to fill in gate commands), list them explicitly so the user knows what to fix before running ai-lore-build-waves.
