...
**Current plugin version: 0.15.1**

---

## Config schema (v0.15.1)
...
Compare the `plugin_version` value in the config (call it `config_version`) against the current plugin version `0.15.1`.

**If `plugin_version` is absent** (v0.1 config -- this field did not exist before 0.4.0):
- This is a non-breaking migration. Auto-patch: add `plugin_version: 0.15.1` at the top of the file (after any leading comment block), then apply all applicable migration files in order. Report what was added or removed.
...
**If `config_version` is a lower minor or patch than `0.15.1`** (e.g. `0.1.x`, `0.4.0`, `0.5.0`):
...
**If `config_version` is a higher version than `0.15.1`**:
...
| 0.15.0 | 0.15.1 | `migrations/0.15.1.md` |
...
3. After scanning all rows:
   ...
   - If both lists are empty and no migration was applied: report "config OK at 0.15.1".
...
3. Create `.ai-lore/` if it does not exist. Write `config.yaml` from `templates/config.yaml` with the detected values filled in and `plugin_version: 0.15.1` set. Report the created file path.
...
End with a one-line status: `config OK at 0.15.1`, `config migrated 0.5.0 -> 0.15.1`, or `config created at 0.15.1`. If any required fields are still missing after migration/creation (e.g. the user declined to fill in gate commands), list them explicitly so the user knows what to fix before running ail-build-waves.
