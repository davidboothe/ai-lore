...
- `brainstorm/<YYYY-MM-DD-slug>/<domain>.md` -- brainstorm domain files (overview, personas, flows, edge-cases, technical, open-questions, team-review, adversarial).
...
Optional first step in the pipeline. Turns a rough feature idea into a structured, diagram-rich brainstorm document ready to hand off to `ail-plan-waves`. The skill interviews the user conversationally across three phases (core concept, mechanics, constraints), writes six focused domain files (overview, personas, flows, edge-cases, technical, open-questions) under `.ai-lore/brainstorm/<slug>/`, then optionally fans out a 5-perspective expert panel (`brainstorm-team.js` Workflow) and a 3-mode adversarial critique (`brainstorm-adversary.js` Workflow). Generates a self-contained HTML preview (CDN mermaid and marked) via `scripts/render-brainstorm.js` after each phase. Supports resume detection: if a brainstorm for the given slug already exists, it reads `brainstorm.yaml` and jumps to the right step. Hands off to `ail-plan-waves` when the user is ready.
