 ...
 *
 * Reads the module and concept docs under a .ai-lore-docs directory, computes the
 * cross-directory dependency graph (depends_on / depended_on_by / concepts, cycles,
 * coupling), and rewrites the managed regions of the module docs plus dependencies.md
 * and index.md. Markdown only; no JSON graph store.
 *
 * The docs are the source of truth. This script is the SOLE writer of the managed
 * module-frontmatter keys (depends_on, depended_on_by, concepts) and the managed body
 * sections (## Concepts, ## Related). It never touches human/LLM-authored prose.
 *
...
const MANAGED_KEYS = ['depends_on', 'depended_on_by', 'concepts'];
const MANAGED_HEADINGS = ['## Concepts', '## Related'];
...
function residual(content) {
  ...
  const keptFm = d.fmLines.filter(function (l) {
    ...
    return !p || MANAGED_KEYS.indexOf(p.key) === -1;
  });
  const keptBody = removeSections(d.bodyLines, MANAGED_HEADINGS);
...
function readDocSet(docsDir) {
  ...
  return { modules: modules, concepts: concepts };
}
...
function renderModuleManaged(record, model) {
  ...
  fmLines = setFmKey(fmLines, 'depends_on', serializeList(deps));
  fmLines = setFmKey(fmLines, 'depended_on_by', serializeList(rdeps));
  fmLines = setFmKey(fmLines, 'concepts', serializeList(cons));
...
function computeOutputs(docsDir, docSet) {
  ...
  return { outputs: outputs, model: model };
}
...
function validate(docsDir, docSet, computed) {
  ...
  docSet.modules.forEach(function (m) {
    if (residual(m.content) !== residual(outputs[m.file])) {
...
function run(docsDir, opts) {
  ...
  if (!fs.existsSync(path.join(absDir, 'modules'))) {
    return { ok: false, errors: ['no modules/ directory under ' + absDir], written: [] };
  }
